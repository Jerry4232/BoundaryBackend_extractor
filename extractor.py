...
# predefined class for geospatial data extraction
...
class GeospatialDataExtractor:
    ...
    def extract_landsat_data(self, collection: str = "landsat_ot_c2_l2",
                         ...
                         download_products: bool = False) -> Dict:
        """
        Extract Landsat data using USGS M2M API

        Args:
            collection: Landsat dataset name
            date_range: Tuple of start and end dates (YYYY-MM-DD)
            download_products: Whether to download selected Landsat products
        """
        try:
            # Default to past 30 days if date_range not given
            if date_range is None:
                end_date = datetime.now()
                start_date = end_date - timedelta(days=30)
                date_range = (start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))

            logger.info("Starting Landsat data extraction...")

            LANDSAT_USERNAME = os.getenv("LANDSAT_USERNAME")
            LANDSAT_API_KEY = os.getenv("LANDSAT_API_KEY")

            if not LANDSAT_USERNAME or not LANDSAT_API_KEY:
                logger.error("Missing USGS credentials.")
                return {"error": "MissingCredentials", "message": "Set LANDSAT_USERNAME and LANDSAT_API_KEY."}

            # Login to USGS M2M API
            login_payload = {"username": LANDSAT_USERNAME, "token": LANDSAT_API_KEY}
            login_response = requests.post(self.api_configs['landsat']['base_url'] + "login-token", json=login_payload)

            if login_response.status_code == 200:
                api_key = login_response.json()["data"]
            else:
                logger.error(f"Login failed: {login_response.status_code} - {login_response.text}")
                return {"error": "LoginFailed", "message": login_response.text}
            ...
            temporal_filter = {"start": date_range[0], "end": date_range[1]}
            ...
            response = requests.post(self.api_configs['landsat']['base_url'] + "scene-search", json=payload, headers=headers)

            if response.status_code != 200:
                logger.error(f"Scene search failed: {response.status_code}")
                return {"error": "SceneSearchFailed", "message": response.text}
            ...
            for product in products:
                product_info = {
                    "id": product.get("displayId"),
                    "acquisitionDate": product.get("acquisitionDate"),
                    "cloudCover": product.get("cloudCover"),
                    "path": product.get("path"),
                    "row": product.get("row"),
                    "sceneBounds": product.get("spatialBounds"),
                }
                processed_products.append(product_info)
            ...
            if processed_products:
                df = pd.DataFrame(processed_products)
                csv_path = self.output_dir / f"{collection}_landsat_products.csv"
                df.to_csv(csv_path, index=False)
                logger.info(f"Product summary saved to {csv_path}")
            ...
            result_dict = {
                ...
                "products_found": len(processed_products),
                "products": processed_products[:5]
            }

            if download_products and processed_products:
                metadata_path = self.output_dir / f"{collection}_landsat_products.json"
                with open(metadata_path, "w") as f:
                    json.dump(processed_products, f, indent=2)
                result_dict["downloads"] = self._download_landsat_products(processed_products[:3], collection)
            ...
            return {
                "error": str(e),
                "collection": collection,
                "message": "Exception during Landsat data extraction"
            }
